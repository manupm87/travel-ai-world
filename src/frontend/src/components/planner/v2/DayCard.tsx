 ...
 * many experiences) over the four parts of the day and their cards.
 */
export function DayCard({ day, date, warnings, defaultOpen = false, onChange, onRemove }: DayCardProps) {
  const { t } = useLanguage();
  ...
  return (
    <Card className="flex flex-col gap-3 p-4">
      ...
      <div id={bodyId} hidden={!expanded} className="flex flex-col gap-4">
        {DAY_PARTS.map((part) => {
                ...
                <ul className="flex flex-col gap-2">
                  {cards.map((card) => {
                    ...
                    return (
                      <li
                        key={card.id}
                        className="flex flex-wrap items-start gap-x-3 gap-y-2 rounded-xl border border-border bg-bg-surface px-3 py-2"
                      >
