...
| `users.ts` | `core_api` (`GET /api/v1/users/me`) | `getMe()` — the signed-in account as core_api knows it (`role`, `subject`, …); `null` on 401 and 404. `AuthContext` calls it once per signed-in account and merges the role (TRA-222) |
| `admin.ts` | `ai_api` (`/api/v1/ai/admin/*`) and `core_api` (`/api/v1/admin/*`), with the session token | The admin console's reads (TRA-222, ADR 0024): `getStats(start, end)`, `listTurns(query, cursor?)` (only the filters that are set: `day`, `kind`, `status`, `subject`, `session`, `trip_id`, `city`, `limit`), `getTurn(id)` (`null` on 404), `listSessionTurns(sessionId, cursor?)`, `listAdminUsers(cursor?)` (200 a page), `listAdminTrips(cursor?)` (50 a page), `getAdminTrip(userId, tripId)` (`null` on 404); the generated types re-exported under short names (`TraceStats`, `TurnSummary`, `AdminUser`, …) |
