...
| `plannerDraft.ts` | `sessionStorage` | The only owner of the planner's per-tab draft: `readPlannerDraft`, `writePlannerDraft`, `clearPlannerDraft` (versioned; a stale or unreadable draft reads as `null`), plus the demo banner's per-tab dismissal (`isDemoBannerDismissed`, `dismissDemoBanner`) |
