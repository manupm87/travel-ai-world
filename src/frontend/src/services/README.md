...
| `trips.ts` | `core_api` for the dashboard (`GET /api/v1/trips/` with the session token); the fixtures in `src/mocks/*.ts` (backend shape) only for the trip viewer, until it moves to the API too | `listTrips({ signal })` (the signed-in user's summaries, `auth: true`, abortable), `getTripById` / `getAllTripIds` (fixtures, viewer only), and the mappers `toTrip` / `toTripSummary` (`TripResponse` → view model, [ADR 0006](../../../../docs/architecture/adr/0006-frontend-trip-view-model.md)) |
