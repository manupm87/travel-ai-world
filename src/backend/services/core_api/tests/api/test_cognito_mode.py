...
from tests.conftest import make_user, trip_body
...
@pytest.fixture
async def cognito_client(table: DynamoTable) -> AsyncGenerator[AsyncClient, None]:
    """A core_api built in Cognito mode, against the test table."""
    app = create_app(
...
async def test_deactivated_account_is_refused_even_with_a_valid_token(
    ...
    await make_user("ada@example.com", is_active=False)
...
async def test_tokens_not_issued_by_the_pool_are_401(
    cognito_client: AsyncClient, users: DynamoUserRepository, token: str
):
...
async def test_a_request_with_the_same_claims_writes_nothing(
    ...
    """The upsert runs on every request; only a change of profile costs a write."""
    token = pool.id_token(sub="sub-ada", email="ada@example.com", name="Ada")
    for _ in range(3):
        me = await cognito_client.get("/api/v1/users/me", headers=bearer(token))
        assert me.status_code == 200, me.text
    ...
    account = await users.get_by_email("ada@example.com")
    assert account is not None
    assert account.version == 0, "created once, never rewritten"

    renamed = pool.id_token(sub="sub-ada", email="ada@example.com", name="Ada L.")
    me = await cognito_client.get("/api/v1/users/me", headers=bearer(renamed))

    assert me.json()["name"] == "Ada L."
    account = await users.get_by_email("ada@example.com")
    assert account is not None
