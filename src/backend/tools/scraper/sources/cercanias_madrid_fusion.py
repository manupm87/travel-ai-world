    ...
    for g_norm, g in google_norm.items():
        if g_norm in wiki_norm:
            ...
            candidatos = difflib.get_close_matches(
                g_norm, wiki_norm.keys(), n=1, cutoff=0.6
            )
            lineas = wiki_norm[candidatos[0]] if candidatos else []

        g["lineas"] = lineas
        g["colores"] = [colores.get(linea) for linea in lineas if linea in colores]
            ...
            items_final.append(
                {
                    ...
                    "lineas": lineas,
                    "colores": [
                        colores.get(linea) for linea in lineas if linea in colores
                    ],
                }
            )
