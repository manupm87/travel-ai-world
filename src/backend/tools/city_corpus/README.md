...
5. **Climate**: monthly highs, lows, rainfall and days with ≥ 1 mm. Open-Meteo's reanalysis
...
uv run python -m city_corpus build budapest \
  [--sources wikivoyage,wikipedia,openstreetmap,wikidata,climate] [--offline] [-v]
```
...
| Field | Meaning |
|---|---|
| `doc_id` | Stable id: `wv:en:Budapest/Belváros#see:parliament` (listing), `wv:en:Budapest/Belváros#section:get-in/from-buda:c1` (prose chunk), `wp:en:3032195#s2-c1` (Wikipedia section 2, chunk 1), `osm:way/47425748`, `om:climate:budapest:07`. A repeated id gets `~2`, `~3` in page order |
...
| `source`, `source_url`, `license`, `lang` | Attribution: `wikivoyage`/`wikipedia` (`CC BY-SA 4.0`), `openstreetmap` (`ODbL 1.0`), `open-meteo` (`CC BY 4.0`); page or element URL; `en`/`es` |
