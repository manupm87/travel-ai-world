...
- **Slash commands** in `.claude/commands/`: `/start-issue`, `/deliver-issue`, `/backend-dev`, `/backend-test`,
...
- **Delivery is a workflow, not a solo session** (the default in `AGENTS.md`, "How work is delivered"):
  the main session audits and writes the brief, then `/deliver-issue TRA-<n>` runs
  `.claude/workflows/kyrian-wave.js` — Opus implements in a worktree and opens the PR, three Sonnet
  reviewers read the diff (correctness, contracts/tests, UX/a11y), Opus fixes and watches CI. Args:
  `{issue, branch, briefPath, checks?, base?}`; resume with `resumeFromRunId`. Sub-agents may not
  edit `.claude/`; the main session does. Runbook: `docs/runbooks/agent-delivery.md`.
