    ...
    <div className={`${bgColor} rounded-lg p-3 flex gap-4 items-start transition-colors hover:bg-white/10 group`}>
      ...
      <div className="flex flex-col gap-0.5 flex-1">
        <div className="flex justify-between items-start gap-2">
          <span className="text-white font-medium">{formatTitle(activity.title)}</span>
          {activity.cost > 0 && (
        ...
        <div className="flex items-center gap-2 mt-1.5">
          {activity.bookingRequired && (
            <span className="text-[10px] uppercase font-medium text-red-400 bg-red-400/10 px-2 py-0.5 rounded tracking-wider">{t.tripViewer.bookingRequired}</span>
          )}
          {activity.rating && (
            <span className="text-[11px] font-medium text-yellow-500 bg-yellow-500/10 px-2 py-0.5 rounded flex items-center gap-1">
